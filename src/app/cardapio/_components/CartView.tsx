...
export default function CartView({ cart, onBack, onRemoveItem, onUpdateQuantity, onClearCart }: Props) {
  ...
  async function handleFinalize() {
    ...
    try {
      await runTransaction(db, async (tx) => {
        ...
        const last = counterSnap.exists() ? (counterSnap.data().last as number) : 0
        const orderNumber = last + 1
        const orderRef = doc(collection(db, 'orders'))
        tx.set(counterRef, { last: orderNumber }, { merge: true })
        tx.set(orderRef, {
          orderNumber,
          status: 'active',
          spot,
          ...
          items: cart.map(({ draftId: _d, ...rest }) => rest),
          subtotal,
          serviceFee,
          total,
    ...
    <div className="min-h-dvh bg-(--bg) sm:bg-[#06080d] sm:flex sm:justify-center">
      <div className="w-full max-w-130 bg-(--bg) min-h-dvh flex flex-col sm:shadow-2xl">
        ...
        <header className="flex items-center px-3 h-14 border-b border-(--border) shrink-0">
          ...
          <span className="text-(--text-primary) font-bold text-lg ml-3">Meu Pedido</span>
        </header>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 pb-10 flex flex-col gap-4">
          ...
          <div className="p-4 rounded-xl border border-(--border) bg-(--bg-surface) flex flex-col gap-1.5">
            <p className="text-(--text-primary) font-bold text-base m-0">Vaga {spot}</p>
            <p className="text-(--text-secondary) text-sm m-0">
              <strong className="text-(--text-primary) font-semibold">Nome:</strong> {name}
            </p>
            <p className="text-(--text-secondary) text-sm m-0">
              <strong className="text-(--text-primary) font-semibold">Telefone:</strong> {phone}
            </p>
          </div>

          {/* Cart items */}
          {cart.length === 0 ? (
            <p className="text-(--text-muted) text-center mt-8 text-sm">Seu pedido está vazio.</p>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-(--text-primary) font-semibold text-sm m-0 leading-snug">{item.name}</p>
                        <button
                      ...
                      {extras.length > 0 && (
                        <p className="text-(--primary) text-[11px] m-0 mt-0.5 leading-snug">
                          {extras.join(' · ')}
                        </p>
                      )}
                      {item.observation && (
                        <p className="text-(--text-muted) text-[11px] m-0 mt-0.5 italic">{item.observation}</p>
                      )}
                      ...
                      <div className="flex items-center gap-2 mt-2.5">
                        ...
                        <span className="text-(--text-primary) font-bold text-sm w-5 text-center">{item.quantity}</span>
                        <button
                          onClick={() => onUpdateQuantity(item.draftId, 1)}
                          className="w-7 h-7 rounded-full border border-(--border) bg-(--bg-elevated) text-(--text-primary) text-base cursor-pointer flex items-center justify-center shrink-0 select-none leading-none"
                        >
          ...
          <div className="p-4 rounded-xl border border-(--border) bg-(--bg-surface) flex flex-col gap-2.5">
            <div className="flex justify-between">
              ...
              <span className="text-(--text-primary) font-semibold text-sm">{formatBRL(subtotal)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-(--text-secondary) text-sm">Taxa de serviço (10%)</span>
              <span className="text-(--primary) font-semibold text-sm">{formatBRL(serviceFee)}</span>
            </div>
            <div className="flex justify-between pt-2.5 border-t border-(--border)">
              ...
              <span className="text-(--text-primary) font-black text-base">{formatBRL(total)}</span>
            </div>
            <p className="text-(--text-muted) text-xs m-0 mt-0.5 flex items-start gap-1.5 leading-snug">
              <BsInfoCircleFill size={11} className="shrink-0 mt-0.5 text-(--primary)" />
              Pagamento efetuado <strong className="text-(--text-secondary)">apenas</strong> na entrega do pedido.
            </p>
          </div>

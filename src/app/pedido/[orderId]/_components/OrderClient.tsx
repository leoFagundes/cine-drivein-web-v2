...
export default function OrderClient({
  ...
  // Track whether the user is intentionally scrolled up reading history
  const userScrolledUp = useRef(false);
  // Ignore the next scroll event triggered programmatically by us
  ...
  // Use a layout effect so the DOM has already painted the new message
  // before we measure scrollHeight — this eliminates the race condition.
  useEffect(() => {
    const newCount = messages.length;
    const added = newCount - prevMsgCount.current;

    if (added <= 0) {
      // Message edited/deleted — don't scroll
      prevMsgCount.current = newCount;
      return;
    }

    const latestMsg = messages[newCount - 1];
    const isMine = latestMsg?.sender === "customer";

    if (isMine) {
      // Always scroll to bottom after sending our own message
      scrollToBottom("smooth");
      userScrolledUp.current = false;
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setUnread(0);
    } else if (!userScrolledUp.current) {
      // Admin message and user hasn't scrolled up → follow the chat
      scrollToBottom("smooth");
      setUnread(0);
    } else {
      // Admin message but user is reading history → show badge
      setUnread((n) => n + added);
    }
  ...
  // Initial scroll to bottom (instant, no animation) on first load
  useEffect(() => {
  ...
  function handleScroll() {
    if (programmaticScroll.current) {
      // This scroll was triggered by us — ignore it
      ...
      <div className="w-full max-w-130 bg-(--bg) h-dvh flex flex-col sm:shadow-2xl">
            ...
            {summaryOpen && (
              <div className="border-t border-(--border) px-4 pt-3 pb-4 flex flex-col gap-3">
                {order.items.map((item, idx) => {
                  const extras = EXTRA_GROUPS.map((g) => ({
                    label: g.label,
                    values: item[g.key] ?? [],
          ...
          <div className="px-4 pt-5 pb-3">
            ...
            {messages.length === 0 && (
              <p className="text-(--text-muted) text-xs text-center py-4 leading-relaxed">
                ...
                <br />
                Envie uma mensagem para a lanchonete!
              </p>
            )}
        ...
        {unread > 0 && (
          <div className="shrink-0 flex justify-center pb-1 bg-(--bg)">
            ...
            >
              {unread} nova{unread > 1 ? "s" : ""} mensagem
              {unread > 1 ? "s" : ""} ↓
            </button>
          </div>
        )}
